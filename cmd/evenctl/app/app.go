...
func cmdMchainAddrBalance(c *cli.Cmd) {
	var (
		token = c.StringOpt("t token", "", "token for blockchain network")
		network = c.StringOpt("n network", "", "name of blockchain network")
		addrList = c.StringsOpt("w wallet", []string{""}, "address list csv")
	)

	c.Spec = "--token ... --network ... --wallet ..."

	c.Action = func() {
		tool.Must(rpc.AddrBalance(*token, *network, *addrList))
	}
}
