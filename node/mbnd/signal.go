...
func interruptListener() <-chan struct{} {
	...
	go func() {
		...
		case sig := <-interruptChannel:
			fmt.Printf("INF: Received signal (%s). Shutting down...", sig)

		case <-shutdownRequestChannel:
			fmt.Println("INF: Shutdown requested. Shutting down...")
		}
		...
		for {
			select {
			case sig := <-interruptChannel:
				fmt.Printf("Received signal (%s).  Already shutting down...", sig)

			case <-shutdownRequestChannel:
				fmt.Println("Shutdown requested.  Already shutting down...")
			}
		}
	}()
