...
func fetchBalance(addr string) (*v.Balance, error) {
	...
	err = db.View(func(dbTx database.Tx) error {
		...
		addrList := make(map[string]btcutil.Address)
		if _, ok := addrList[addr]; ok != true {
			addrList[addr], err = btcutil.DecodeAddress(addr, chainConfig.ChainParams)
			if err != nil {
				return err
			}
		}
...
func fetchMempoolTxnsForAddr(addr btcutil.Address,
	numToSkip,
